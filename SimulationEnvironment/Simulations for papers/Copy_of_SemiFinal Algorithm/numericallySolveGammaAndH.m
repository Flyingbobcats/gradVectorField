...
% title(str)
title(strcat('v=',num2str(v),{'   '},'r_0 = ',num2str(obstR),{'   '}, 'y_o=',{' '}, num2str(obstY),{'   '},'cost = ',num2str(cost)));
set(gca,'fontsize',12);
xlabel('East (m)');
ylabel('North (m)');
...
%             str = strcat('alpha = ',num2str(rad2deg(alpha)),{' '},'zeta =',num2str(rad2deg(atan2(Y_turn,X_turn))));
%         disp(str);
%         pause();
%     If inside avoidance region, weight functions
    if abs(uav.x)<= abs(X)+velocity*dt*2 && Alpha > atan2(Y_turn-obstY,X_turn)
        ...
        g = -uav.v*cos(beta)-abs(1/((range-obstR)));
        g = -(velocity+abs(1/(10*(range-obstR))))*cos(beta);
        g = -(abs(1/((range-obstR))))*cos(beta);
        
        g = 1*cos(beta+pi);%-1/exp((range-obstR));
%         g = 0;

%         g = -(1+abs(1/((range-obstR))))*cos(abs(beta));
%         g = -(1+abs(velocity/((range-obstR))))*cos(abs(beta));
        
%         g = -uav.v*cos(abs(beta))
%         if g>0
%             g = 0;
%         end
        vf.rvf{1}.G = g;
